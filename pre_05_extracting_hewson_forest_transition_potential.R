...
# hewson_forest_transition_potential has NAs in non-forest areas
# Turning them to zero (making clear this layer is about forest loss)
downrc[is.na(downrc)] <- 0
plot(downrc)
downrc2 <- mask(downrc, target)
downrcc <- crop(downrc2, tr)
plot(downrcc)
...
writeRaster(downrcc, filename = 'hewson_forest_transition_potential.tif', format="GTiff", overwrite=TRUE)
