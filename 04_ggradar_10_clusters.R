...
clusters <- read.csv("clusters_rgeoda_c10.csv")
...
barplot(c(1:10), col=pal)
barplot(c(1), col="#85D4E3")
barplot(c(1:10), col=rev(pal)) 
...
g$grobs[[52]]$grobs[[1]]$children[[1]]$gp$fill 

strip_both <- c(57,58,59,60,61,52,53,54,55,56 )
...
cridges <- joyc %>% 
  ...
  facet_wrap(~cluster , nrow = 2, ncol=5)+
...
# Understanding grob slots - varies according to cluster size to be plottes

gr$grobs[[62]]$grobs[[1]]$children[[1]]$gp$fill 

strip_bothr <- c(57,58,59,60,61,52,53,54,55,56 )
fillsr <- unique(pal)
...
png("facet_clusters_ridges_10.png",
    width = 28,
    height = 25, unit='cm', res=600)
